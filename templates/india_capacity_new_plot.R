# create capacity additions plots
...
# inputs -----------------
  ...
  data_fname      = '_capacity_new_data_all_zones.csv'
...
# ------------------------------- plot -------------------------------
  ...
  # country level plots -------
    ...
    fig_capacity = ggplot(capacity_tot, aes(x = as.character(period), y = capacity_mw/1e3, group = type, fill = type)) + 
      ...
      labs(title = paste0('New capacity', ' - scenario: ', sel_scenario),
           subtitle = 'GW',
           x = NULL,
           y = NULL, 
           fill = NULL) +
      scale_y_continuous(labels = scales::comma, expand = c(0,0), breaks = seq(0, 10e2, 1e2), limits = c(NA, 10e2)) +
      scale_fill_manual(values = pal_fuel) +
      theme_line
    
    fname = paste0('capacity_new_by_technology__', 'India', '.pdf')
    
    dir.create(here::here('scenarios', db_dir, sel_scenario, 'results', 'figures', 'capacity-new-plots'), showWarnings = FALSE)
    
    ggsave(fig_capacity, 
           filename = here::here('scenarios', db_dir, sel_scenario, 'results', 'figures', 'capacity-new-plots', fname), 
           width = 9.8, 
           height = 6.25)
    
    embed_fonts(here::here('scenarios', db_dir, sel_scenario, 'results', 'figures', 'capacity-new-plots', fname),
                outfile = here::here('scenarios', db_dir, sel_scenario, 'results', 'figures', 'capacity-new-plots', fname))
  ...
  for (i in loadzones) {
    ...
    fig_capacity = ggplot(data, aes(x = as.character(period), y = capacity_mw, group = type, fill = type)) + 
      geom_bar(position = 'stack', stat = 'identity') +
      labs(title = paste0('New capacity for ', i, ' - scenario: ', sel_scenario),
           subtitle = 'MWh',
           x = NULL,
           y = NULL, 
           fill = NULL) +
    ...
    fname = paste0('capcity_new_by_technology_', i, '.pdf')
