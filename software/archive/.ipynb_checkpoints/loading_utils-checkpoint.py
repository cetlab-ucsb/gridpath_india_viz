...
def _load_new_and_existing_capacity(scenarios_, zones_, path):
...
def _group_capacity_technologies(capacity_all_, tech_groups_):
...
def _group_dispatch_technologies(ed_, tech_groups_):
...
# Grab data from databases for plotting LCOE emissions
def _load_system_cost(scenarios_, zones_, path, single_node = False):
...
# Grab data from databases for plotting GHG emissions
def _load_GHG_emissions(scenarios_, zones_, path):
